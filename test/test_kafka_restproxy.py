...
class Test(unittest.TestCase):
    ...
    def test_group_offsets(self):
        r = RestProxy(config_str)
        r.enable_auto_commit(False)
        r.commit_after_processing(False)
        #
        topic_str = self.create_test_topic_name()
        ...
        consumer.commit()
        #
        group_str_topic_str_offsets_dict_dict_dict = r.group_offsets(group_str)
    ...
    def test_topics(self):
        ...
        topic_str_size_int_partitions_dict_dict = r.topics(pattern=topic_str, size=True, partitions=True)
        ...
        topic_str_partitions_dict_dict = r.l(pattern=topic_str, size=False, partitions=True)
    ...
    def test_partitions_set_partitions(self):
        ...
        topic_str = self.create_test_topic_name()
        r.create(topic_str, partitions=2)
        partitions_int1 = r.partitions(topic_str)[topic_str]
    ...
    def test_produce_consume_bytes_str(self):
        ...
        producer = r.producer(topic_str, key_type="bytes", value_type="str")
    ...
    def test_produce_consume_json(self):
        ...
        producer = r.producer(topic_str, key_type="json", value_type="json")
    ...
    def test_produce_consume_protobuf(self):
        ...
        producer = r.producer(topic_str, key_type="protobuf", value_type="pb", key_schema=self.protobuf_schema_str, value_schema=self.protobuf_schema_str)
        producer.produce(self.snack_dict_list, key=self.snack_str_list)
        producer.close()
    ...
    def test_produce_consume_avro(self):
        ...
        producer = r.producer(topic_str, key_type="avro", value_type="avro", key_schema=self.avro_schema_str, value_schema=self.avro_schema_str)
    ...
    def test_produce_consume_jsonschema(self):
        ...
        producer = r.producer(topic_str, key_type="json_sr", value_type="jsonschema", key_schema=self.jsonschema_schema_str, value_schema=self.jsonschema_schema_str)
        producer.produce(self.snack_dict_list, key=self.snack_str_list)
        producer.close()
        self.assertEqual(r.topics(topic_str, size=True, partitions=True)[topic_str]["partitions"][0], 3)
        #
    ...
    def test_head(self):
        ...
        topic_str = self.create_test_topic_name()
        r.create(topic_str)
        producer = r.producer(topic_str, value_type="avro", value_schema=self.avro_schema_str)
        producer.produce(self.snack_str_list)
        producer.close()
        #
        group_str = self.create_test_group_name()
    ...
    def test_tail(self):
        ...
        topic_str = self.create_test_topic_name()
        r.create(topic_str)
        producer = r.producer(topic_str, value_type="protobuf", value_schema=self.protobuf_schema_str)
    ...
    # Shell.cp -> Functional.map_to -> Functional.flatmap_to -> RestProxyr.producer/Functional.foldl/RestProxyConsumer.close -> RestProxyConsumer.consumer/KafkaConsumer.foldl/RestProxyConsumer.close -> RestProxyConsumer.consume
    def test_cp(self):
        ...
        r.create(topic_str1)
        producer = r.producer(topic_str1, value_type="json_sr", value_schema=self.jsonschema_schema_str)
    ...
    def test_diff(self):
        ...
        topic_str1 = self.create_test_topic_name()
        r.create(topic_str1)
        w1 = r.producer(topic_str1, value_type="protobuf", value_schema=self.protobuf_schema_str)
        w1.produce(self.snack_str_list)
        w1.close()
        #
        topic_str2 = self.create_test_topic_name()
        r.create(topic_str2)
        w2 = r.producer(topic_str2, value_type="avro", value_schema=self.avro_schema_str)
    ...
    def test_foreach(self):
        ...
        topic_str = self.create_test_topic_name()
        r.create(topic_str)
        producer = r.producer(topic_str, value_type="jsonschema", value_schema=self.jsonschema_schema_str)
        producer.produce(self.snack_str_list)
        producer.close()
        #
        colour_str_list = []
    ...
    def test_filter_to(self):
        ...
        topic_str1 = self.create_test_topic_name()
        r.create(topic_str1)
        producer = r.producer(topic_str1, value_type="avro", value_schema=self.avro_schema_str)
        producer.produce(self.snack_str_list)
        producer.close()
        #
        topic_str2 = self.create_test_topic_name()
